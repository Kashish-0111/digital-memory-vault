...
import {
  Plus, Search, Pin, Lock, Unlock, Trash2, Edit3, Mic, MicOff, Tag, Filter,
} from 'lucide-react';
...
export default function NotesPage() {
  const {
    notes, categories,
    addNoteFirestore, updateNoteFirestore, deleteNoteFirestore,
    firebaseUser,
  } = useApp();
  ...
  const { isListening, transcript, error, startListening, stopListening, resetTranscript, isSupported } = useVoiceInput();
  ...
  const filteredNotes = useMemo(() => {
    ...
    return result;
  }, [notes.state.notes, notes.state.searchQuery, notes.state.filterCategory]);

  const openAddModal = () => {
    setEditingNote(null);
    setFormData({ title: '', description: '', category: categories[0]?.name || '', tags: '' });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingNote) {
      // Update locally
      notes.dispatch({ type: notesActions.UPDATE_NOTE, payload: { id: editingNote.id, data } });
      // Update Firestore if Firebase user
      if (firebaseUser) {
        await updateNoteFirestore(editingNote.id, data);
      }
    } else {
      if (firebaseUser) {
        // Add to Firestore first — get the Firestore ID
        const firestoreId = await addNoteFirestore(data);
        notes.dispatch({
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            ...
            {firebaseUser && (
              <span className="ml-2 text-xs text-green-500">● Cloud Synced</span>
            )}
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            onChange={(e) => notes.dispatch({ type: notesActions.SET_SEARCH, payload: e.target.value })}
        ...
        <div className="relative">
          ...
          <select
            ...
            onChange={(e) => notes.dispatch({ type: notesActions.SET_FILTER, payload: e.target.value })}
            ...
            >
              ...
              <div className="mb-3">
                {note.isPrivate && !showPrivate[note.id] ? (
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-3">{note.description}</p>
                )}
              </div>
              ...
              {note.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {note.tags.map((tag) => (
                    <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-slate-700">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => togglePin(note.id)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-400 hover:text-primary-500 transition-colors" title={note.isPinned ? 'Unpin' : 'Pin'}>
                    <Pin className={`w-4 h-4 ${note.isPinned ? 'fill-primary-500 text-primary-500' : ''}`} />
                  </button>
                  <button onClick={() => togglePrivate(note.id)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-400 hover:text-primary-500 transition-colors">
                    {note.isPrivate ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}
                  </button>
                  <button onClick={() => openEditModal(note)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-400 hover:text-primary-500 transition-colors">
                    <Edit3 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(note.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-gray-400 hover:text-red-500 transition-colors">
              ...
              {note.isPrivate && (
                <button
                  onClick={() => setShowPrivate((prev) => ({ ...prev, [note.id]: !prev[note.id] }))}
                  className="mt-2 text-xs text-primary-600 dark:text-primary-400 hover:underline flex items-center gap-1"
                >
                  {showPrivate[note.id] ? <><Lock className="w-3 h-3" /> Hide content</> : <><Unlock className="w-3 h-3" /> View content</>}
                </button>
              )}
            </GlassCard>
          ))}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingNote ? 'Edit Note' : 'Add Note'}>
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {isSupported && (
                <button
                  ...
                  className={`ml-2 inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg transition-colors ${
                    isListening ? 'bg-red-100 text-red-600 dark:bg-red-900/30' : 'bg-gray-100 text-gray-600 dark:bg-slate-700 dark:text-gray-400'
                  }`}
                >
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="btn-primary">{editingNote ? 'Update' : 'Add'} Note</button>
          </div>
        </form>
      </Modal>
    </div>
